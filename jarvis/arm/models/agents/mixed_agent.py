...
class MixedAgent(BaseAgent):
    ...
    def forward(self, 
        ...
        # state shape: (batch, agent_count, memory, hidsize)
        outputs = None
        new_states = []
        vpred = {}
        ...
        for i, key in enumerate(self.ordered_sub_keys):
            sub_inputs = obs.copy()
            sub_inputs['obs_conf'] = obs_conf.get(key, {})
            ...
            action_head = self.sub_agents[key].action_head()
            pi_logits = action_head(latents['pi_latent'])
            output = TensorDict(pi_logits)
            if outputs is None:
