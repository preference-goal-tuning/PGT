...
class MinecraftAgentPolicy(nn.Module):
    ...
    def __init__(
        ...
        self.net:GrootPolicy = policy_cls(state_space=state_space, action_space=action_space, **policy_body_kwargs)
        self.useful_heads = {}
        self.make_value_head(self.net.output_latent_size())
        self.make_action_heads(self.net.output_latent_size(), **pi_head_kwargs)
        self.auxiliary_heads = build_auxiliary_heads(
    ...
    def forward(
        ...
        **kwargs
        ...
        latents, state_out, internal_loss = self.net.forward(
            obs=obs, 
            state_in=state_in, 
            context={"first": first}, 
        ...
        if stage != 'rollout':
            for head, module in self.auxiliary_heads.items():
                result[head] = module(latents, stage=stage, useful_heads=self.useful_heads)
        
        return result, state_out, latents

    def set_var(self, key, val):
        if key == 'KL_weight':
            assert 'prior_regular_head' in self.auxiliary_heads, "KL_weight is only supported when prior_regular_head is enabled!"
            self.auxiliary_heads['prior_regular_head'].weight = val
        elif key == 'FSQ_weight':
            self.net.condition_encoder.z_space.weight = val
        else:
            raise ValueError(f"Unknown key: {key}")

    def get_var(self, key):
        if key == 'z_space':
            return self.net.condition_encoder.z_space
        else:
            raise ValueError(f"Unknown key: {key}")
    ...
    def get_metrics(self):
        ...
        for head, module in self.auxiliary_heads.items():
            for subname in dir(module):
                ...
                if isinstance(submodule, Metric):
                    metrics += [submodule]
        return metrics