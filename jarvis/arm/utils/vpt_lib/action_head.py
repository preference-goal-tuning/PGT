...
class MSEActionHead(ActionHead):
    ...
    def sample(self, pd_params: torch.Tensor, deterministic: bool = False) -> torch.Tensor:
        assert deterministic, "MSE action head does not support non-deterministic sampling"
        return pd_params
...
class DLMLActionHead(ActionHead):
    ...
    def forward(self, input_data: torch.Tensor, mask=None) -> Dict[str, torch.Tensor]:
        assert not mask, "Can not use a mask in a mse action head"
    ...
    def sample(self, pd_params, deterministic: bool = False) -> torch.Tensor:
