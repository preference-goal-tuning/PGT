...
import struct
import jarvis.stark_tech.env.database_manager as database_manager
...
class _MultiAgentEnv(gym.Env):
            ...
            for role, (actor_name, instance) in enumerate(zip(self.task.agent_names, self.instances)):
                try:  # TODO - we could wrap entire function in try, if sockets don't need to individually clean
                    ...
                    if not self.has_finished[actor_name]:
                        ...
                        if done:
                            self.logger.info("Agent {} has finished".format(actor_name))
    ...
    def _send_mission(self, instance: MinecraftInstance, mission_xml_etree: etree.Element, token_in: str) -> None:
        ...
        ok = 0
        num_retries = 0
        self.logger.debug("Sending mission init: {instance}".format(instance=instance))
        while ok != 1:
            ...
            if ok != 1:
                ...
                self.logger.debug("Recieved a MALMOBUSY from {}; trying again.".format(instance))
                time.sleep(1)

    def _peek_obs(self):
        multi_obs = {}
        if not self.done:
            self.logger.debug("Peeking the clients.")
            peek_message = "<Peek/>"
            multi_done = True
    ...
    def close(self):
        """gym api close"""
        self.logger.debug("Closing MineRL env...")
    ...
    def _TO_MOVE_create_connection(self, instance: MinecraftInstance) -> None:
        try:
            self.logger.debug("Creating socket connection {instance}".format(instance=instance))
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
            sock.settimeout(SOCKTIME)
            sock.connect((instance.host, instance.port))
            self.logger.debug("Saying hello for client: {instance}".format(instance=instance))
            self._TO_MOVE_hello(sock)

            instance.client_socket = sock
        except (socket.timeout, socket.error, ConnectionRefusedError) as e:
            instance.had_to_clean = True
            self.logger.error("Failed to reset (socket error), trying again!")
            self.logger.error("Cleaning connection! Something must have gone wrong.")
            self._TO_MOVE_clean_connection(instance)
    ...
    def _TO_MOVE_quit_current_episode(self, instance: MinecraftInstance) -> None:
        ...
        self.logger.info("Attempting to quit: {instance}".format(instance=instance))
        # while not has_quit:
    ...
    def _TO_MOVE_find_ip_and_port(self, instance: MinecraftInstance, token: str) -> Tuple[str, str]:
        ...
        self.logger.info("Attempting to find_ip: {instance}".format(instance=instance))
        ...
        self.logger.warning("MineRL agent is public, connect on port {} with Minecraft 1.11".format(port))
