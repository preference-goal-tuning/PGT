...
class Worker(object):
    ...
    def move_to_pos(self, x: float, y: float, speed: float = 25):
        camera_x = x - self.cursor[0]
        camera_y = y - self.cursor[1]
        distance =max(abs(camera_x), abs(camera_y))
        num_steps= int(np.random.uniform(5, 10) * math.sqrt(distance) / speed)
        if num_steps < 1:
            num_steps = 1
    ...
    def random_move_or_stay(self):
        if np.random.uniform(0, 1) > 0.5:
            ...
            if np.random.uniform(0, 1) > 0.25:
                ...
                for i in range(num_random):
                    d1 =  np.random.uniform(-5, 5)
                    d2 =  np.random.uniform(-5, 5)
                    self.action_once(d1, d2)
        else:
            pass
    ...
    def crafting(self, target: str, target_num: int=1, is_recipe=False, close_inventory=False):
        # recipe info
        try:
            self.outframes = []
            self.outactions = []
            ...
            if not self.current_gui_type:
                if need_table:
                    if is_recipe:
                        self.open_crating_table_w_recipe()
                        self.crafting_slotpos = SLOT_POS_TABLE_W_RECIPE
                    else:
                        self.open_crating_table_wo_recipe()
                        self.crafting_slotpos = SLOT_POS_TABLE_WO_RECIPE
                else:
                    if is_recipe:
                        self.open_inventory_w_recipe()
                        self.crafting_slotpos = SLOT_POS_INVENTORY_W_RECIPE
                    else:
                        self.open_inventory_wo_recipe()
                ...
                if need_table:
                    if 'table' in self.current_gui_type:
                        ...
                        labels = self.get_labels()
                        if labels['inventory_0']['type'] == 'crafting_table':
                            self._call_func('inventory')
                        if is_recipe:
                            self.open_crating_table_w_recipe()
                            self.crafting_slotpos = SLOT_POS_TABLE_W_RECIPE
